...
public class BodyCollider : MonoBehaviour {
	...
	private IEnumerator OnTriggerEnter(Collider other)
	{
		if(other.tag =="Ground"){
			// クラッシュした時の処理
			Debug.Log("Crash");
			board_obj.GetComponent<Rigidbody>().AddForce(new Vector3(0,100,0),ForceMode.Impulse);
			board_obj.transform.FindChild("Human").transform.FindChild("Capsule").GetComponent<Collider>().isTrigger = false;
			board_obj.transform.FindChild("Human").transform.FindChild("Sphere").GetComponent<Collider>().isTrigger = false;
